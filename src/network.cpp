...
{
    enforce_condition(!layer_info.empty(),
        "Network::Network - layer_info vector cannot be empty");
...
LPP::Network::Vec LPP::Network::inference(const Vec& x) const
{
...
LPP::Network::Vec LPP::Network::inference(const std::span<const float> x) const
{
    inference_buffer_.assign(x.begin(), x.end());
...
)
{
    ...
    float  learning_rate         = init_learning_rate;
           loss_func_            = loss_ptr;
           X_i_                  = Vec(training_features.cols());
           Y_i_                  = Vec(training_responses.cols());
    ...
    if (options.use_validation()) estimated_validation_responses_ = Matrix(options.validation_responses().rows(), options.validation_responses().cols());
    ...
    num_batches = num_training_examples / mini_batch_size;
    ...
    Matrix              estimated_training_responses(training_features.rows(), training_responses.cols());
    std::vector<Matrix> del_W(layers_.size());
    std::vector<Vec>    del_b(layers_.size());
    ...
    for (size_t cur_epoch = 0; cur_epoch < epochs; cur_epoch++) {
        ...
        for (size_t cur_mini_batch = 0; cur_mini_batch < num_batches; cur_mini_batch++) {
            ...
            train_on_batch_(del_W, del_b, batch_start_idx, batch_end_idx,
                training_features, training_responses, estimated_training_responses,
                permutation);

            // This step changes layer weights and biases!
            update_parameters_(del_W, del_b, actual_batch_size, learning_rate, options.regularizer());
        }

        float training_loss, validation_loss;
        float training_accuracy, validation_accuracy;
        compute_losses_(options,
            training_loss, validation_loss, training_accuracy, validation_accuracy,
            training_responses, estimated_training_responses);
        print_losses_(options, training_loss, validation_loss, training_accuracy, validation_accuracy, cur_epoch + 1);
    }
}
...
void LPP::Network::initialize_gradient_sizes_(
    ...
    for (size_t cur_layer_idx = 0; cur_layer_idx < layers_.size(); cur_layer_idx++) {
        ...
        delL_delb_partial_sum[cur_layer_idx] = Vec(out, 0.0);
    }
}
...
*/
void LPP::Network::train_on_batch_(
    ...
    const Matrix& training_features,
    const Matrix& training_responses,
    Matrix& estimated_training_responses,
    ...
    for (size_t t = start; t < end; t++) {
        ...
        estimated_training_responses.set_row(
            idx, forward_propagation_(X_i_, true));
...
float LPP::Network::validation_loss_(
    ...
    // This can be parallelized if we make inference_buffer_ thread local
    for (size_t t = 0; t < validation_responses.rows(); t++)
    {
        estimated_validation_responses_.set_row(
            t, inference(validation_features[t]));
    }
    // Return validation loss
    return loss_func_->apply_loss(estimated_validation_responses_, validation_responses);
}

void LPP::Network::compute_losses_(const ExtraTrainingOptions& options, float& train_loss, float& val_loss, float& train_acc, float& val_acc, const Matrix& y, const Matrix& y_hat) const {
    // Compute loss from it
    float training_data_loss = loss_func_->apply_loss(y_hat, y);
    // Compute loss from penalty
    ...
    train_loss = training_data_loss + penalty_loss;
    ...
    if (options.use_validation()) {
        val_loss = validation_loss_(options.validation_features(), options.validation_responses());
...
void LPP::Network::print_losses_(const ExtraTrainingOptions& options, float train_loss, float val_loss, float train_acc, float val_acc, size_t epoch) const
{
    if (options.has_output_stream()) {
        auto& os = options.output_stream();
        
        if (options.use_mini_batch()) os << '\n';
        os << "\tTraining Loss: " << train_loss << '\n';
        if (options.use_validation()) os << "\tValidation Loss: " << val_loss << '\n';
        if (options.performs_classification()) os << "\tTraining Accuracy: " << train_acc << '\n';
        if (options.performs_classification() && options.use_validation()) os << "\tValidation Accuracy: " << val_acc << '\n';
        os << '\n';
    }
    if (options.has_metadata_stream()) {
        ...
        ms << epoch << ' ' << train_loss;
        if (options.use_validation()) ms << ' ' << val_loss;
        if (options.performs_classification()) ms << ' ' << train_acc;
        if (options.performs_classification() && options.use_validation()) ms << ' ' << val_acc;
        ms << '\n';
    }
}
